...
- **Settings…** — cửa sổ cài đặt dạng sidebar-tabs (giống System Settings), chia theo chức năng:
  ...
  - **Scrolling**: **hướng cuộn riêng cho chuột và trackpad**. macOS chỉ có duy nhất 1 công tắc "Natural scrolling" dùng chung cho mọi thiết bị — chỉnh đúng cho trackpad thì chuột bị ngược, và ngược lại. Bật **Reverse Scroll Direction**, rồi chọn đảo **Reverse Mouse** (mặc định bật) và/hoặc **Reverse Trackpad** (mặc định tắt): giữ setting hệ thống đúng cho một thiết bị, Tide đảo lại thiết bị còn lại. Phân biệt chuột với trackpad theo đúng thiết bị phát ra event (đọc IORegistry), nên Magic Mouse — vốn cũng gửi scroll dạng pixel giống trackpad — vẫn được nhận đúng là chuột. Chỉ đảo trục dọc; cần cấp quyền **Accessibility** (System Settings → Privacy & Security → Accessibility), bật xong là chạy ngay, không cần khởi động lại app.
